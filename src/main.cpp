      ...
      if (length && length > 2 && data[0] == '4' && data[1] == '2') {
        ...
        if (s != "") {
          ...
          if (event == "telemetry") {
            ...
            if (pre_path_size <
                2) { //the previous calculated path almost end, use the car's current position as starting reference
            ...
            vector<double> next_wp0 = getXY(car_s + 30, (2 + 4 * lane_id), map_waypoints_s, map_waypoints_x,
                                            map_waypoints_y);
            vector<double> next_wp1 = getXY(car_s + 60, (2 + 4 * lane_id), map_waypoints_s, map_waypoints_x,
                                            map_waypoints_y);
            vector<double> next_wp2 = getXY(car_s + 90, (2 + 4 * lane_id), map_waypoints_s, map_waypoints_x,
                                            map_waypoints_y);
            ...
            double N = tgt_dist / (tgt_vel / mps2mph *
                                   0.02); //tgt_dist is in meters, 0.02s is the simulation excution time
